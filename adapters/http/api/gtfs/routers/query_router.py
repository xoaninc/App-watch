...
def get_route_shape(
    ...
    smooth: Optional[int] = Query(
        ...
        ge=0,
        le=5,
        description="Apply Chaikin corner-cutting smoothing. Value is number of iterations (0=disabled, 1-2 for subtle, 3+ for very smooth). Default: 2 for subtle smoothing."
    ),
    ...
    **Smoothing (smooth parameter):**
    When `smooth` is specified, applies Chaikin's corner-cutting algorithm
    to round angular corners in the shape. This is useful for:
    - Shapes generated by BRouter that have sharp angles
    - Low-resolution shapes with few points
    - Creating a more modern "metro map" aesthetic

    Example: `?smooth=2` applies 2 iterations of smoothing (recommended for subtle effect).
    ...
    **Combined usage:**
    You can combine both parameters: `?smooth=2&max_gap=50` first smooths
    corners, then densifies the result.
    ...
    **Stop positions (include_stops parameter):**
    ...
    # Apply Chaikin smoothing if requested (before normalization)
    if smooth and points:  # smooth=0 or None skips smoothing
        # Extract just lat/lon for smoothing
        coords = [(lat, lon) for lat, lon, _ in points]
        smoothed_coords = smooth_shape_chaikin(coords, iterations=smooth)
        # Rebuild with new sequence numbers
        points = [(lat, lon, seq) for seq, (lat, lon) in enumerate(smoothed_coords, 1)]
